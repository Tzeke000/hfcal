...
const _MGRS = (function() {
  ...
  function decode(mgrsString) {
    ...
    while (i < length && (/[A-Z]/).test(testChar = mgrsString.charAt(i)) && (hunK === null ? ++i : false) !== false) {
      if (hunK === null) { hunK = ''; }
      hunK += testChar;
      if (hunK.length === 2) { i++; break; }
    }
...
function parseMGRS(raw) {
  try {
    var clean = raw.trim().toUpperCase().replace(/[EN](?=\s|$)/g, '').replace(/\s+/g, '');
    var pt = _MGRS.toPoint(clean);
    return { lon: pt[0], lat: pt[1], error: null };
  } catch(e) {
    return { lon: NaN, lat: NaN, error: 'Invalid MGRS: ' + e.message };
  }
}

function looksLikeMGRS(s) {
...
function parseCoords(raw) {
  ...
  if (m) {
    ...
    else if (lon2 > 180) lon2 = lon2 - 360;
    return { lat: lat2, lon: lon2, error: null };
  }
  ...
  if (m) {
    ...
    else if (lon3 > 180) lon3 = lon3 - 360;
    return { lat: lat3, lon: lon3, error: null };
  }
  ...
  if (m) {
    ...
    else if (lon4 > 180) lon4 = lon4 - 360;
    return { lat: lat4, lon: lon4, error: null };
  }
  ...
  if (m) {
    ...
    else if (lon5 > 180) lon5 = lon5 - 360;
    return { lat: lat5, lon: lon5, error: null };
  }
  ...
  if (m) {
    ...
    else if (lon6 > 180) lon6 = lon6 - 360;
    return { lat: lat6, lon: lon6, error: null };
  }
  ...
  if (m) {
    ...
    if (lon7 > 180) lon7 = lon7 - 360;
    return { lat: lat7, lon: lon7, error: null };
  }
