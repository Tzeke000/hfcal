...
def main():
    ...
    json.dump({'sites': SITES, 'test_sites': sorted(TEST_SITES),
               ...
               'note': ('the SHIPPED figure is the interpolated lookup, written '
                        'into MFACTOR_HELDOUT_PCT in src/data/mfactorTable.js; these '
...
def export_js():
    """Emit src/data/mfactorTable.js from the saved rows.
    ...
    js = [
        ...
        '//   regenerate: python3 scripts/validation/build/build_mfactor_table.py --export',
        '//',
    ...
    open(path, 'w').write('\n'.join(js))
    print('wrote src/data/mfactorTable.js (%d cells, %.1f KB)'
